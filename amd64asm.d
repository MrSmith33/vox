...
enum sizeofReg(R) = [1, 2, 4, 8][staticIndexOf!(R, AliasSeq!(Reg8, Reg16, Reg32, Reg64))];
...
struct Imm8  { ubyte  value; alias Reg = Reg8 ; string toString(){ return format("0X%02X", value); } }
struct Imm16 { ushort value; alias Reg = Reg16; string toString(){ return format("0X%02X", value); } }
struct Imm32 { uint   value; alias Reg = Reg32; string toString(){ return format("0X%02X", value); } }
struct Imm64 { ulong  value; alias Reg = Reg64; string toString(){ return format("0X%02X", value); } }
...
MemAddress memAddrDisp32(uint disp32) {
	return MemAddress(MemAddrType.disp32, Reg32.ESP, Reg32.EBP, SibScale(), disp32); } // 0b00
MemAddress memAddrIndexDisp32(Reg32 indexReg, SibScale scale, uint disp32) {
	return MemAddress(MemAddrType.indexDisp32, indexReg, Reg32.EBP, scale, disp32); } // 0b00
...
MemAddress memAddrBaseDisp32(Reg32 baseReg, uint disp32) {
	return MemAddress(MemAddrType.baseDisp32, Reg32.ESP, baseReg, SibScale(), disp32); } // 0b10
...
MemAddress memAddrBaseIndexDisp32(Reg32 baseReg, Reg32 indexReg, SibScale scale, uint disp32) {
	return MemAddress(MemAddrType.baseIndexDisp32, indexReg, baseReg, scale, disp32); } // 0b10
MemAddress memAddrBaseDisp8(Reg32 baseReg, ubyte disp8) {
	return MemAddress(MemAddrType.baseDisp8, Reg32.ESP, baseReg, SibScale(), disp8); } // 0b01
MemAddress memAddrBaseIndexDisp8(Reg32 baseReg, Reg32 indexReg, SibScale scale, ubyte disp8) {
	return MemAddress(MemAddrType.baseIndexDisp8, indexReg, baseReg, scale, disp8); } // 0b01
...
struct CodeGen_x86_64(Sink)
{
	...
	private void putInstrRegImm(R, I)(ubyte opcode, R dst_rm, I src_imm) if (isAnyReg!R && isAnyImm!I) {
		static assert(sizeofReg!R == I.value.sizeof, "Sizes of imm and reg must be equal");
		static if (is(R == Reg16)) sink.put(LegacyPrefix.OPERAND_SIZE);         // 16 bit operand prefix
		putRexByte_B(dst_rm);                                                   // REX
		sink.put(opcode | (dst_rm & 0b0111));                                   // Opcode
		sink.put(src_imm);                                                      // Imm
	}
	private void putInstrRegMem(R)(ubyte opcode, R dst_r, MemAddress src_mem) if (isAnyReg!R) {
		static if (is(R == Reg16)) sink.put(LegacyPrefix.OPERAND_SIZE);         // 16 bit operand prefix
	...
	private void putInstrMemImm(I)(ubyte opcode, MemAddress dst_mem, I src_imm) if (isAnyImm!I) {
		static if (is(I.Reg == Reg16)) sink.put(LegacyPrefix.OPERAND_SIZE);     // 16 bit operand prefix
		putRexByte_RXB(cast(I.Reg)0, cast(I.Reg)dst_mem.indexReg, cast(I.Reg)dst_mem.baseReg);// REX
		sink.put(opcode);                                                       // Opcode
		...
		static if (is(I == Imm64)) {
			Imm32 imm32 = Imm32(cast(uint)src_imm.value);
			assert(src_imm.value == imm32.value, "Imm64 must only contain 32-bit values");
			sink.put(imm32);                                                      // Mem64 Imm32
		} else {
	...
	void mov(Reg8  dst_reg, MemAddress src_rm) { putInstrRegMem(0x8A, dst_reg, src_rm); }
	void mov(Reg16 dst_reg, MemAddress src_rm) { putInstrRegMem(0x8B, dst_reg, src_rm); }
	void mov(Reg32 dst_reg, MemAddress src_rm) { putInstrRegMem(0x8B, dst_reg, src_rm); }
	void mov(Reg64 dst_reg, MemAddress src_rm) { putInstrRegMem(0x8B, dst_reg, src_rm); }

	void mov(MemAddress dst_rm, Reg8  src_reg) { putInstrRegMem(0x88, src_reg, dst_rm); }
	void mov(MemAddress dst_rm, Reg16 src_reg) { putInstrRegMem(0x89, src_reg, dst_rm); }
	void mov(MemAddress dst_rm, Reg32 src_reg) { putInstrRegMem(0x89, src_reg, dst_rm); }
	void mov(MemAddress dst_rm, Reg64 src_reg) { putInstrRegMem(0x89, src_reg, dst_rm); }

	void mov(MemAddress dst_rm, Imm8  src_imm) { putInstrMemImm(0xC6, dst_rm, src_imm); }
	void mov(MemAddress dst_rm, Imm16 src_imm) { putInstrMemImm(0xC7, dst_rm, src_imm); }
	void mov(MemAddress dst_rm, Imm32 src_imm) { putInstrMemImm(0xC7, dst_rm, src_imm); }
	// Imm64 can only use 32bit values here, it is sign extended to 64-bits
	void mov(MemAddress dst_rm, Imm64 src_imm) { putInstrMemImm(0xC7, dst_rm, src_imm); }
